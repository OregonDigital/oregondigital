RSpec.configure do |config|
  config.before(:each) do
    stub_request(:get, "http://purl.org/dc/dcmitype/Image").
      ...
      to_return(:status => 200, :body => lambda {|request| "<#{request.uri.to_s.gsub(":80","")}> <http://purl.org/dc/terms/title> \"Image\"@en .'"}, :headers => {:'Content-Type' => 'application/n-triples'})
  end
end
